...
In this study we aim to build a bioinformatic pipeline that addresses the issues of paralogy when processing transcriptomic data from non-model organisms, utilises alternative methods to both concatenation and maximum likelihood inference. Specifically, the pipeline assembles RNA seq datasets, identifies and extracts single copy genes across input taxa with extensive paralogy, and runs Bayesian inference phylogenetics. Further, to demonstrate decrease in inference run time on GPU vs. CPU. Finally, to validate the adequacy of the pipeline though application to the historically difficult order Gonyaulacales (phylum: Dinoflagellata) which are notorious for the issues that the pipeline seeks to address.

%protists and environmental importance
Dinoflagellates represent an ancient lineage on the eukaryotic branch of life. %Elucidating the evolution of the dinoflagellates is key to answering a number of questions. 
...
While the evolutionary relationship of most orders within the dinoflagellates has been estimated with reasonable certainty, one order has consistently escaped scrutiny - the gonyaulacales. Analyses for this order commonly combined concatenation and maximum likelihood approaches, which yielded long branch attraction, low confidence values and inconsistent taxon resolution. Improving upon the species the evolution of the gonyaulaceles has implications for environmental disease monitoring, as neurotoxin production is prevalent in this order. 
%A common type of toxin found within the dinoflagellates, especially the gonyaulacales, are polyketides. 
